  ...
  const initialize = useCallback(async () => {
    try {
      ...
      if (accessToken && isValidToken(accessToken)) {
        ...
        const response = { data: { user: everton } };
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    const response = {
      data: {
        user: everton,
        accessToken:
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJleHAiOjE3MzUyMzkwMjJ9.AcTTRQXywO79Uw011ruikMyU2zOkc8SEK6ho60xnr-c",
      },
    };
